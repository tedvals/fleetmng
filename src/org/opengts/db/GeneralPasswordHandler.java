    ...
    implements PasswordHandler
{
    ...
    **/
        ...
        throws PasswordEncodingException
    {
        ...
        if (encType.equalsIgnoreCase("md5plain")) {
            this.encoding   = ENC_MD5;
        ...
        if (encType.equalsIgnoreCase("plain")) {
    ...
    **/
    protected Object getProperty(String key)
    {
        if (this.rtProps.hasProperty(key)) {
            return this.rtProps.getProperty(key, "");
        } else {
    ...
    **/
    ...
    {
        if (this.rtProps.hasProperty(key)) {
            return this.rtProps.getString(key, "");
        } else {
    ...
    **/
    ...
    {
        if (this.rtProps.hasProperty(key)) {
            return this.rtProps.getLong(key, 0L);
        } else {
            return DefaultProps.getLong(key, 0L);
        }
    }
    ...
    **/
    ...
    {
        if (this.rtProps.hasProperty(key)) {
            return this.rtProps.getInt(key, 0);
        } else {
            return DefaultProps.getInt(key, 0);
        }
    }
    ...
    **/
    ...
    {
        if (this.rtProps.hasProperty(key)) {
            return this.rtProps.getBoolean(key, false);
        } else {
            return DefaultProps.getBoolean(key, false);
        }
    }
    ...
    public boolean checkPassword(String enteredPass, String tablePass) 
    {
        ...
        if (this.isEncoding(ENC_MD5) && (tablePass.length() != MD5_HASH_LEN) && this.checkPlain) {
            if (tablePass.equals(enteredPass)) {
                //Print.logInfo("Login OK: MD5/Test special case match");
                return true;
            } else {
                //Print.logInfo("Login Failed: MD5/Test special case did not match");
                return false;
            }
        }

        /* failed */
        //Print.logInfo("Login Failed: No match");
    ...
    **/
    public boolean validateNewPassword(String newPass) 
    {
        ...
        /* minimum length */
        int minLen = this.getInt(PROP_minimumPasswordLength);
        ...
        /* category count */
        int minCategories = this.getInt(PROP_minimumCategories);
        if (minCategories > 0) {
            int catCnt = 0;
    ...
    **/
    public String getPasswordFormatDescription(Locale locale) 
    {
        ...
        int minLower = this.getInt(PROP_minimumLowerAlphaChars);
        if (minLower > 0) {
        ...
        int minUpper = this.getInt(PROP_minimumUpperAlphaChars);
        if (minUpper > 0) {
        ...
        /* min upper-alpha characters */
        int minAlpha = this.getInt(PROP_minimumAlphaChars);
        if (minAlpha > 0) {
        ...
        int minDigit = this.getInt(PROP_minimumDigitChars);
        if (minDigit > 0) {
        ...
        int minSpec = this.getInt(PROP_minimumSpecialChars);
        if (minSpec > 0) {
        ...
        int minNonAlpha = this.getInt(PROP_minimumNonAlphaChars);
        if (minNonAlpha > 0) {
    ...
    **/
    ...
    {
        long maxAgeSec = this.getLong(PROP_maximumPasswordAgeSeconds);
