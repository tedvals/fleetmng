    ...
    implements EventDataProvider, GeoPointProvider
{
  ...
  //public static final String FLD_obdType              = "obdType";                // OBD type [0,1=J1708, 2=J1939, 3=OBDII]
    ...
    public static final String FLD_massAirFlowRate      = "massAirFlowRate";        // L/sec
    ...
    public static final DBField CANBUSFieldInfo[] = { 
      ...
      //new DBField(FLD_obdType          , Integer.TYPE  , DBField.TYPE_UINT16     , I18N.getString(EventData.class,"EventData.fld.obdType"            , "OBD Type"              ), ""),
        ...
        new DBField(FLD_massAirFlowRate  , Double.TYPE   , DBField.TYPE_DOUBLE     , I18N.getString(EventData.class,"EventData.fld.massAirFlowRate"    , "Mass Air Flow Rate"    ), "format=#0.00 units=volume"),
  ...
  //public static final String FLD_appliedPressure      = "appliedPressure";        // kPa
    ...
    **/
        ...
        throws SlowOperationException
    {
        ...
        double    evLon = this.getLongitude();
        ...
        Geozone gzone = Geozone.getGeozone(acct.getAccountID(), null, addrGP, true); // <Geozone>.getReverseGeocode() == true
        if (gzone != null) {
            Set<String> updFields = new HashSet<String>();
    ...
    public void setOutputMask(long v)
    {
        if (v < 0L) {
            ...
            this.setFieldValue(FLD_outputMask, (v & 0xFFFFFFFFL)); // 32-bit
        }
    }
    ...
    */
    ...
    {
        return this.getFieldValue(FLD_massAirFlowRate, 0.0); // L/sec
    }
    ...
    **/
    ...
    /* returns true if MalfunctionLamp was explicitly set */
    ...
    **/
    ...
    public static final String DEBUG_PUSHPINS[] = new String[] { "DebugPushpins", "debugPP" };
    ...
    **/
        ...
        boolean isFleet, BasicPrivateLabel bpl)
    {
        ...
        if (this.explicitPushpinIconIndex >= 0) {
            if (debugPP) PushpinDebug("Explicit index", iconKeys, this.explicitPushpinIconIndex);
        ...
        if (this.iconIndexProvider != null) {
            ...
            if (iconNdx >= 0) {
                if (debugPP) PushpinDebug("IconIndexProvider", iconKeys, iconNdx);
                return iconNdx;
            }
        }
        ...
        //    Check selected by "iconSelector" (typically from the Status Code specification)

        /* IconSelector: check rule factory */
        if (!StringTools.isBlank(iconSelector)) {
            RuleFactory rf = Device.getRuleFactory(true);
            if (rf != null) {
                int iconNdx = this._getPushpinIndexFromRuleSelector(rf, "IconSelector", iconSelector, iconKeys, 
                    debugPP);
                if (iconNdx >= 0) {
        ...
        if (isFleet) {
            ...
            // ---------------------------------------
            // 4.a) If "trackMap.lastDevicePushpin.fleet=true", use Device pushpin
            ...
            if (this.showLastEventDevicePushpin(isFleet,bpl)) {
                ...
                if ((dev != null) && dev.hasPushpinID()) {
                    ...
                    if (iconNdx >= 0) {
                        if (debugPP) PushpinDebug("Device ("+dev.getDeviceID()+") fleet", iconKeys, iconNdx);
                        return iconNdx;
                    }
                }
            }
            ...
            if (fleetIconNdx >= 0) {
                if (debugPP) PushpinDebug("\""+PPNAME_fleet+"\" name", iconKeys, fleetIconNdx);
                return fleetIconNdx;
            }
        ...
        if (allIconNdx >= 0) { // '0' is a valid index
            if (debugPP) PushpinDebug("\""+PPNAME_all+"\" name", iconKeys, allIconNdx);
            return allIconNdx;
        }
        ...
        if (!isFleet && this.getIsLastEvent()) {
            ...
            if (this.showLastEventDevicePushpin(!isFleet,bpl)) {
                ...
                if ((dev != null) && dev.hasPushpinID()) {
                    ...
                    if (devIconNdx >= 0) {
                        if (debugPP) PushpinDebug("Device ["+dev.getDeviceID()+"] last", iconKeys, devIconNdx);
                        return devIconNdx;
                    }
                }
            }
            ...
            if (lastIconNdx >= 0) { // '0' is a valid index
                if (debugPP) PushpinDebug("\""+PPNAME_last+"\" name", iconKeys, lastIconNdx);
                return lastIconNdx;
            }
        ...
        if (this.isStopEvent(false)) { // check for specific "STOP" status code
            ...
            if (stopIconNdx >= 0) { // '0' is a valid index
                if (debugPP) PushpinDebug("\""+PPNAME_stop+"\" name", iconKeys, stopIconNdx);
                return stopIconNdx;
            }
        }
        ...
        if (this.getSpeedKPH() > 0.0) { // check for non-zero speed
            ...
            if (movingIconNdx >= 0) { // '0' is a valid index
                if (debugPP) PushpinDebug("\""+PPNAME_moving+"\" name", iconKeys, movingIconNdx);
        ...
        if (dftIconNdx >= 0) {
            if (debugPP) PushpinDebug("default(\""+dftIconName+"\") name", iconKeys, dftIconNdx);
            return dftIconNdx;
        }
        ...
        if (debugPP) PushpinDebug("Not Found", iconKeys, ICON_PUSHPIN_BLACK);
        ...
        boolean debugPP)
    {
        ...
        if (!StringTools.isBlank(scIconName)) {
            ...
            if (iconNdx >= 0) {
                if (debugPP) PushpinDebug("StatusCode name", iconKeys, iconNdx);
                return iconNdx;
            }
        }
        ...
        if (rf != null) {
            ...
            int iconNdx = this._getPushpinIndexFromRuleSelector(rf, "StatusCode", scIconSel, iconKeys, 
                debugPP);
            if (iconNdx >= 0) {
                // already displays Pushpins debug
                return iconNdx;
    ...
    *** @param acctID      The Account ID
    *** @param devID       The Device ID
    *** @param oldTime     The time in the past before which (exclusive) events will be deleted.  
    *** @return The number of events deleted.
    **/
    public static long deleteOldEvents(
        String acctID, String devID,
        ...
        throws DBException
    {
        ...
        if (lastTimestamp <= oldTime) {
            // last event is prior to delete time threashold.  Move 'oldTime" to
            // just prior to last timestamp in order to save at elast one event
            // for this device.
            oldTime = lastTimestamp; // deletion is non-inclusive
            Print.logInfo("(Saving last event for device: " + acctID + "/" + devID + ")");
        }

        /* count events in range */
        long count = EventData.getRecordCount(acctID,devID,-1L,(oldTime - 1L));
        if (count <= 0L) {
            // already empty range
            return 0L;
        }
        ...
        ddel.setWhere(dwh.WHERE_(
            dwh.AND(
                dwh.EQ(EventData.FLD_accountID,acctID),
                dwh.EQ(EventData.FLD_deviceID ,devID),
                dwh.LT(EventData.FLD_timestamp,oldTime) // non-inclusive
            )
        ));
    ...
    public  static final String KEY_DRIVER_DESC[]   = new String[] { "driver"           , "driverDesc"     };  // "Joe Smith"
    ...
    private static final String KEY_INPUT_MASK[]    = new String[] { "inputMask"                           };  // ""
    private static final String KEY_INPUT_BIT[]     = new String[] { "inputBit"                            };  // "Off"
    private static final String KEY_OUTPUT_MASK[]   = new String[] { "outputMask"                          };  // ""
        ...
        Locale locale, BasicPrivateLabel bpl, EventData ed)
    {
        ...
        if (EventData._keyMatch(key,EventData.KEY_ACCOUNT)) {
            if (getTitle) {
                ...
                return (account != null)? account.getDescription() : ed.getAccountID();
            }
        } else
        ...
        if (EventData._keyMatch(key,EventData.KEY_DEVICE)) {
            if (getTitle) {
                ...
                return (device != null)? device.getDescription() : ed.getDeviceID();
            }
        } else
        ...
        if (EventData._keyMatch(key,EventData.KEY_DIRECTION)) {
            if (getTitle) {
                ...
                return GeoPoint.GetHeadingString(ed.getHeading(),locale);
            }
        } else
        if (EventData._keyMatch(key,EventData.KEY_HEADING)) {
            if (getTitle) {
                ...
                return StringTools.format(ed.getHeading(),"0.0");
            }
        } else
        if (EventData._keyMatch(key,EventData.KEY_ODOMETER)) {
            if (getTitle) {
                ...
                double  odomKM  = ed.getOdometerKM();
                Device  device  = ed.getDevice();
                if (device != null) {
        ...
        if (EventData._keyMatch(key,EventData.KEY_FUEL_LEVEL)) {
            if (getTitle) {
                ...
                long pct = Math.round(ed.getFuelLevel() * 100.0);
                return pct + "%";
            }
        } else
        if (EventData._keyMatch(key,EventData.KEY_FUEL_LEVEL2)) {
            if (getTitle) {
                ...
                long pct = Math.round(ed.getFuelLevel2() * 100.0);
                return pct + "%";
            }
        } else
        if (EventData._keyMatch(key,EventData.KEY_FUEL_VOLUME)) {
            if (getTitle) {
                ...
                if (device == null) {
                    ...
                    double L = device.getFuelCapacity() * ed.getFuelLevel();
        ...
        if (EventData._keyMatch(key,EventData.KEY_DRIVER_DESC)) {
            if (getTitle) {
                ...
                if (!StringTools.isBlank(driverID)) {
                    Driver driver = ed.getDriver();
                    if (driver != null) {
                        return driver.getDescription();
                    } else {
                        Print.logError("Unable to read Driver: " + driverID);
                        return driverID;
                    }
                }
                // next try EntityID
