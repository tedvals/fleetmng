    ...
    implements UserInformation
{
    ...
    **/
    ...
    public enum EconomyUnits implements EnumTools.StringLocale, EnumTools.IntValue {
        ...
        public String  toString(Locale loc)         { return aa.toString(loc); }
      //public double  getMultiplier()              { return mm; }
        public double  convertFromKPL(double v)     { return ii? (1.0 / (v * mm)) : (v * mm); }
        public double  convertToKPL(double v)       { return ii? ((1.0 / v) / mm) : (v / mm); }
    };
        ...
        Locale locale, BasicPrivateLabel bpl, Account acct)
    {
        ...
        if (EventData._keyMatch(key,EventData.KEY_DATETIME)) {
            if (getTitle) {
                ...
                return EventData.getTimestampString(now, acct, bpl);
            }
        } else
        if (EventData._keyMatch(key,EventData.KEY_DATE_YEAR)) {
            if (getTitle) {
                ...
                return EventData.getTimestampYear(now, acct);
            }
        } else
        if (EventData._keyMatch(key,EventData.KEY_DATE_MONTH)) {
            if (getTitle) {
                ...
                return EventData.getTimestampMonth(now, false, acct, locale);
            }
        } else
        if (EventData._keyMatch(key,EventData.KEY_DATE_DAY)) {
            if (getTitle) {
                ...
                return EventData.getTimestampDayOfMonth(now, acct);
            }
        } else
        if (EventData._keyMatch(key,EventData.KEY_DATE_DOW)) {
            if (getTitle) {
                ...
                return EventData.getTimestampDayOfWeek(now, false, acct, locale);
            }
        } else
            ...
            try {
                ...
                for (String acctListID : acctList) {
                    ...
                    if (StringTools.isBlank(type)     || 
                        ...
                        asb.append(" [active=").append(account.isActive());
                        ...
                        if (!type.equalsIgnoreCase("count")) {
                            for (Device device : devList) {
                                ...
                                boolean actv = device.getIsActive();
                                StringBuffer dsb = new StringBuffer();
                                dsb.append("  Device: [ID=").append(device.getDeviceID());
                                ...
                                if (eventCnt) {
                                    ...
                                    if (!ListTools.isEmpty(lastEv)) {
                                        ...
                                        Print.sysPrintln("    > Last Event: " + lastEventDt);
                                    } else {
