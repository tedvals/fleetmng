    ...
    extends AbstractClientPacketHandler
{
    ...
    //
    // Format # 9: (see "parseInsertRecord_RTProps_9")
    ...
    private boolean parseInsertRecord_Common(GPSEvent gpsEv)
    {
        ...
        /* estimate GPS-based odometer */
        Device device = gpsEv.getDevice(); // guaranteed non-null
        double odomKM = 0.0; // set to available odometer from event record
        //if (device.getLastEventTimestamp() < fixtime) {
        if (odomKM <= 0.0) {
    ...
    private byte[] parseInsertRecord_ASCII_01(String s)
    {
        ...
        this.sessionID = DCServerFactory.createTcpSessionID(device);
    ...
    private byte[] parseInsertRecord_Device_1(String s)
    {
        ...
        double   longitude  = validGPS? Nmea0183.ParseLongitude(fld[ 6], fld[6], 180.0) : 0.0;
