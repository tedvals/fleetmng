            ...
            if (!allowNew) {
               newUser = false; // not authorized
                ...
                if (StringTools.isBlank(newUserID)) {
                    m = i18n.getString("UserInfo.enterNewUser","Please enter a new user ID.");
                    error = true;
                    newUser = false;
                } else
                ...
                try {
                    ...
                    if (!isCurrentUserSelected) {
                        ...
                        if (this.isValidPassword(privLabel,userPassword)) {
                            selUser.setDecodedPassword(Account.encodePassword(privLabel,userPassword));
                        } else {
                    ...
                    // ACL Role
                    if (showRole && 
                        privLabel.hasWriteAccess(currUser,this.getAclName(_ACL_ROLE))) {
                    ...
                    // ACLs
                    if (showACLs && !ListTools.isEmpty(aclKeys) && 
                        privLabel.hasWriteAccess(currUser,this.getAclName(_ACL_ACLS))) {
