    ...
    implements DBRecordHandler<EventData>
{
    ...
    private static final String PROP_WorHours_              = "WorkHours.";
    ...
    **/
    public void postInitialize()
    {
        ...
        if (this.tabulateByWorkHours) {
            this.workHours = new WorkHours(this.getProperties(), PROP_WorHours_);
    ...
    **/
    public DBDataIterator getBodyDataIterator()
    {
            ...
            try {
                ...
                if (this.lastStopTime > 0) {
                    ...
                    this._addRecord(accountID, devID, 
                        ...
                        driveTime, driveDist, fuelTrip, driveEcon,
        ...
        if (this.deviceCount > 1) {
            ...
            fd.setAccount(account);
            fd.setString(FieldLayout.DATA_ACCOUNT_ID         , this.getAccountID());
            fd.setDouble(FieldLayout.DATA_ODOMETER_DELTA     , grandTotalOdomKM);
    ...
    **/
    private void _addRecord(String acctID, String devID, 
        ...
        long driveTime , double driveDist, double fuelTrip    , double driveEcon,
        ...
        )
    {
        ...
        if (this.tabulateByWorkHours && (this.workHours != null)) {
            whStart = (startTime > 0L)? this.workHours.isMatch(startTime, this.timeZone) : false;
            whStop  = (stopTime  > 0L)? this.workHours.isMatch(stopTime , this.timeZone) : false;
            ...
            if ((startTime <= 0L) && (stopTime > 0L)) {
                // no start time
                boolean stopWH = this.workHours.isMatch(stopTime , this.timeZone);
                if (stopWH) {
            ...
            if ((startTime > 0L) && (stopTime <= 0L)) {
                // no stop time
                boolean startWH = this.workHours.isMatch(startTime, this.timeZone);
                if (startWH) {
                    // all time/distance attributed to WorkHours
                    whDriveTime = driveTime;
                    whDriveDist = driveDist;
            ...
            if (startTime < stopTime) {
                ...
                double accumHR = this.workHours.countWorkHours(startDT, stopDT, this.timeZone);
                if (accumHR > 0.0) {
