    ...
    extends CommonServlet
{
    ...
    private static String PARM_COMMAND[]                    = { "cmd"                               };  // Command
    private static String PARM_MOBILE[]                     = { "id"   , "imei", "un"               };  // MobileID
    private static String PARM_ACCOUNT[]                    = { "acct" , "account", "a"             };  // AccountID
    private static String PARM_DEVICE[]                     = { "dev"  , "device", "d"              };  // DeviceID
    private static String PARM_AUTH[]                       = { "pass" , "password", "pw"           };  // authorization/password
    ...
    private static String PARM_ODOMETER[]                   = { "odom" , "odometer"                 };  // odometer (kilometers)
    ...
    private static String PARM_DATE[]                       = { "date" , "dt"                       };  // date (YYYYMMDD)
    private static String PARM_TIME[]                       = { "time" , "tm"                       };  // time (HHMMSS)
    ...
    private static String PARM_SPEED[]                      = { "speed", "kph"                      };  // speed (kph)
    private static String PARM_HEADING[]                    = { "head" , "heading", "direction"     };  // heading (degrees)
    ...
    static {
        ...
        if (!UniqueIDPrefix.endsWith("_") && !UniqueIDPrefix.endsWith("-")) {
            UniqueIDPrefix += "_";
        }
        ...
        PARM_SPEED         = RTConfig.getStringArray(CONFIG_PARM_SPEED    , PARM_SPEED)    ;
        ...
        PARM_ODOMETER      = RTConfig.getStringArray(CONFIG_PARM_ODOMETER , PARM_ODOMETER) ;
        ...
        throws ServletException, IOException
    {
        ...
        String        mobileID   = AttributeTools.getRequestString(request, PARM_MOBILE    , null);
        ...
        if (isPost) {
            ...
            if (cmd.equalsIgnoreCase("version") || 
                ...
                this.plainTextResponse(response, "OK:version:"+DEVICE_CODE+"-"+VERSION);
                return;
            }
        }
        ...
        if (!StringTools.isBlank(mobileID)) {
            ...
            String uid = UniqueIDPrefix + mobileID; // ie: "gprmc_123456789012345"
            try {
                ...
                Data.logError(null, "Exception getting Device: " + mobileID + " [" + dbe + "]");
                this.plainTextResponse(response, RESPONSE_ERROR);
                return;
            }
